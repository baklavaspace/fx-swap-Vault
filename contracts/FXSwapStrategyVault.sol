...
contract FXSwapStrategyVault is
    Initializable,
    UUPSUpgradeable,
    BaseVault
{
    ...
    address internal constant WFX = 0x3452e23F9c4cC62c70B7ADAd699B264AF3549C19;     // WFX mainnet: 0x80b5a32E4F032B2a058b4F29EC95EEfEEB87aDcd; WFX testnet: 0x3452e23F9c4cC62c70B7ADAd699B264AF3549C19
    address internal constant BAVA = 0xc7e56EEc629D3728fE41baCa2f6BFc502096f94E;    // BAVA mainnet: 0xc8B4d3e67238e38B20d38908646fF6F4F48De5EC; BAVA testnet: 0xc7e56EEc629D3728fE41baCa2f6BFc502096f94E
    ...
    function deposit(uint256 _assets, address _receiver) public nonReentrant override returns (uint256) {
        ...
        if (restakingEnabled == true) {
            uint256 stakeAmount = IERC20Upgradeable(asset()).balanceOf(address(this));
    ...
    function emergencyRedeem() external nonReentrant {
        ...
        IERC20Upgradeable(asset()).safeTransfer(address(msg.sender), assets);
    ...
    function updateBonusReward(IERC20Upgradeable[] memory _bonusRewardTokens)
        public
        onlyRole(GOVERNOR_ROLE)
    {
        bonusRewardTokens = _bonusRewardTokens;
    ...
    function _convertBonusRewardIntoWFX() private {
        ...
        if (rewardLength > 0) {
            ...
            for (uint256 i; i < rewardLength; i++) {
                if (address(bonusRewardTokens[i]) != address(WFX)) {
